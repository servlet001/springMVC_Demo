(function($) {
    ...
    window.liger = $.ligerui = {
        ...
        getId:function(prev) {
            ...
            var id = prev + (1e3 + this.managerCount);
            this.managerCount++;
            return id;
        },
        ...
        run:function(plugin, args, ext) {
            ...
            if (typeof args[0] == "string") {
                ...
                if (args[0] == "option") {
                    if (args.length == 2) return manager.get(args[1]); else if (args.length >= 3) return manager.set(args[1], args[2]);
                } else {
                    var method = args[0];
    ...
    liger.win = {
        ...
        addTask:function(win) {
            ...
            task.bind("click", function() {
                self.activeTask(win);
                if (win.actived) win.min(); else win.active();
            }).hover(function() {
    ...
    liger.core.Win.ligerExtend(liger.core.UIComponent, {
        ...
        mask:function() {
            if (this.options.modal) liger.win.mask(this);
        },
        unmask:function() {
            if (this.options.modal) liger.win.unmask(this);
        },
        min:function() {},
        max:function() {},
        active:function() {}
...
(function($) {
    liger.inject = {
        ...
        fotmatValue:function(value, type) {
            ...
            if (type == "number" && value) return parseFloat(value.toString());
            return value;
        },
        getOptions:function(e) {
            ...
            for (var proName in defaults) {
                ...
                if (subElement.length) {
                    ...
                    if (typeof subDefaults == "string") subDefaults = g.parseDefault(subDefaults); else if (typeof subDefaults == "funcion") subDefaults = subDefaults();
                    if (typeof subConfig == "string") subConfig = g.parse(subConfig); else if (typeof subConfig == "funcion") subConfig = subConfig();
                    if (subDefaults) {
    ...
    $.ligerui.controls.Accordion.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render:function() {
            ...
            $(".l-accordion-toggle", g.accordion).hover(function() {
                if ($(this).hasClass("l-accordion-toggle-open")) $(this).addClass("l-accordion-toggle-open-over"); else if ($(this).hasClass("l-accordion-toggle-close")) $(this).addClass("l-accordion-toggle-close-over");
            }, function() {
                if ($(this).hasClass("l-accordion-toggle-open")) $(this).removeClass("l-accordion-toggle-open-over"); else if ($(this).hasClass("l-accordion-toggle-close")) $(this).removeClass("l-accordion-toggle-close-over");
            });
        ...
        onResize:function() {
            ...
            if (g.accordion.parent()[0].tagName.toLowerCase() == "body") {
                ...
                g.height = p.heightDiff + windowHeight * parseFloat(g.height) * .01;
            } else {
                g.height = p.heightDiff + g.accordion.parent().height() * parseFloat(p.height) * .01;
            }
            g.accordion.height(g.height);
...
(function($) {
    ...
    $.ligerui.controls.Button.ligerExtend($.ligerui.controls.Input, {
        ...
        _setEnabled:function(value) {
            if (value) this.button.removeClass("l-button-disabled");
        },
...
(function($) {
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render:function() {
            ...
            if (p.absolute) g.selectBox.appendTo("body").addClass("l-box-select-absolute"); else g.textwrapper.append(g.selectBox);
        ...
        setTree:function(tree) {
            ...
            tree.onAfterAppend = function(domnode, nodedata) {
                ...
                var value = null;
                if (p.initValue) value = p.initValue; else if (g.valueField.val() != "") value = g.valueField.val();
                g.selectValueByTree(value);
            };
            g.tree = $("<ul></ul>");
        ...
        _addClickEven:function() {
            ...
            $(".l-table-nocheckbox td", g.selectBox).click(function() {
                ...
                if (g.hasBind("beforeSelect") && g.trigger("beforeSelect", [ value, text ]) == false) {
                    if (p.slide) g.selectBox.slideToggle("fast"); else g.selectBox.hide();
                    return false;
                }
                if ($(this).hasClass("l-selected")) {
                    if (p.slide) g.selectBox.slideToggle("fast"); else g.selectBox.hide();
        ...
        updateSelectBoxPosition:function() {
            ...
            if (p.absolute) {
                ...
                if (Number(g.wrapper.offset().top + 1 + g.wrapper.outerHeight() + g.selectBox.height()) > contentHeight && contentHeight > Number(g.selectBox.height() + 1)) {
                    g.selectBox.css({
...
(function($) {
    ...
    $.ligerui.controls.DateEditor.ligerExtend($.ligerui.controls.Input, {
        ...
        _render:function() {
            var g = this, p = this.options;
            if (!p.showTime && p.format.indexOf(" hh:mm") > -1) p.format = p.format.replace(" hh:mm", "");
            ...
            if (p.absolute) g.dateeditor.appendTo("body").addClass("l-box-dateeditor-absolute"); else g.textwrapper.append(g.dateeditor);
            ...
            g.buttons.btnYear.click(function() {
                if (!g.body.yearselector.is(":visible")) {
                    $("li", g.body.yearselector).each(function(i, item) {
                        ...
                        if (currentYear == g.currentDate.year) $(this).addClass("l-selected"); else $(this).removeClass("l-selected");
            ...
            g.buttons.btnMonth.click(function() {
                $("li", g.body.monthselector).each(function(i, item) {
                    if (g.currentDate.month == i + 1) $(this).addClass("l-selected"); else $(this).removeClass("l-selected");
                });
            ...
            g.toolbar.time.hour.click(function() {
                $("li", g.body.hourselector).each(function(i, item) {
                    if (g.currentDate.hour == i) $(this).addClass("l-selected"); else $(this).removeClass("l-selected");
                });
            ...
            g.toolbar.time.minute.click(function() {
                $("li", g.body.minuteselector).each(function(i, item) {
                    if (g.currentDate.minute == i) $(this).addClass("l-selected"); else $(this).removeClass("l-selected");
                });
    ...
    l.controls.Dialog.ligerExtend(l.core.Win, {
        ...
        _render:function() {
            ...
            if (!p.fixedType) {
                ...
                if (p.left != null) left = p.left; else p.left = left = .5 * ($(window).width() - width);
                if (p.top != null) top = p.top; else p.top = top = .5 * ($(window).height() - g.dialog.height()) + $(window).scrollTop() - 10;
        ...
        collapse:function() {
            ...
            if (p.slide) g.dialog.content.animate({
                height:1
            }, p.slide); else g.dialog.content.height(1);
            if (this.resizable) this.resizable.set({
                disabled:true
            });
        },
        extend:function() {
            ...
            }, p.slide); else g.dialog.content.height(contentHeight);
            if (this.resizable) this.resizable.set({
                disabled:false
            });
        },
    ...
    l.controls.Drag.ligerExtend(l.core.UIComponent, {
        ...
        _stop:function(e) {
            ...
            if (g.proxy) {
                if (p.revert) {
                    if (g.hasBind("revert")) {
                        if (g.trigger("revert", [ g.current, e ]) != false) g._revert(e); else g._removeProxy();
                    } else {
                        g._revert(e);
                    }
                } else {
        ...
        _applyDrag:function(applyResultBody) {
            ...
            if (g.current.diffX) {
                if (noproxy || p.proxyX == null) cur.left = g.current.left + g.current.diffX; else cur.left = g.current.startX + p.proxyX + g.current.diffX;
                changed = true;
            }
            if (g.current.diffY) {
                if (noproxy || p.proxyY == null) cur.top = g.current.top + g.current.diffY; else cur.top = g.current.startY + p.proxyY + g.current.diffY;
                changed = true;
            }
        ...
        _setReceive:function(receive) {
            this.receiveEntered = {};
            if (!receive) return;
            if (typeof receive == "string") this.receive = $(receive); else this.receive = receive;
        },
        _setHandler:function(handler) {
            var g = this, p = this.options;
            if (!handler) g.handler = $(p.target); else g.handler = typeof handler == "string" ? $(handler, p.target) :handler;
        },
...
(function($) {
    ...
    $.ligerDefaults.Filter.editors["number"] = {
        ...
        getValue:function(input, field) {
            ...
            if (isInt) return parseInt(input.val(), 10); else return parseFloat(input.val());
        },
        destroy:function(input) {
            input.liger("destroy");
        }
...
(function($) {
    ...
    $.ligerDefaults.Form.editorBulider = function(jinput) {
        ...
        if (ltype == "autocomplete") options.autocomplete = true;
        if (jinput.is("select")) {
            ...
            if (options.readonly) {
                if (field.textField) {
                    ...
                    if (p.appendID) textInput.attr("id", field.textField);
                ...
                switch (ltype) {
                  ...
                  case "popup":
                    ...
                    if (!textInput.length) textInput = $("<input type='text' name='" + textField + "' />").insertAfter(jinput);
                    if (p.appendID) textInput.attr("id", textField);
                    if (ltype == "popup") textInput.ligerPopupEdit(options); else textInput.ligerComboBox(options);
                    break;
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setFields:function(fields) {
            ...
            if (fields && fields.length) {
                ...
                $(fields).each(function(index, field) {
                    if ($.inArray(field.group, groups) == -1) groups.push(field.group);
                });
                ...
                $(".l-group", jform).width(jform.width() * .95).append("<div class='togglebtn'></div>");
            }
        ...
        _buliderControl:function(field, fieldIndex) {
            var g = this, p = this.options;
            var width = field.width || p.inputWidth, name = field.name || field.id, type = (field.type || "text").toLowerCase(), readonly = field.readonly || field.editor && field.editor.readonly ? true :false;
            var out = [];
            if (type == "textarea" || type == "htmleditor") {
                out.push("<textarea ");
            } else if ($.inArray(type, [ "checkbox", "radio", "password" ]) != -1) {
    ...
    $(".l-form .l-group .togglebtn").live("click", function() {
        if ($(this).hasClass("togglebtn-down")) $(this).removeClass("togglebtn-down"); else $(this).addClass("togglebtn-down");
...
(function($) {
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setFrozen:function(frozen) {
            if (frozen) this.grid.addClass("l-frozen"); else this.grid.removeClass("l-frozen");
        },
        ...
        _setHeight:function(h) {
            ...
            if (typeof h == "string" && h.indexOf("%") > 0) {
                if (p.inWindow) h = $(window).height() * parseFloat(h) * .01; else h = g.grid.parent().height() * parseFloat(h) * .01;
            }
            if (p.title) h -= 24;
            if (p.usePager) h -= 32;
        ...
        loadData:function(loadDataParm) {
            ...
            if (p.dataType == "local") {
                ...
                if (p.usePager) g.currentData = g._getCurrentPageData(g.filteredData); else {
            ...
            var ajaxOptions = {
                ...
                success:function(data) {
                    ...
                    if (p.dataAction == "server") {
                        ...
                        g.filteredData = g.data;
                        if (clause) g.filteredData[p.root] = g._searchData(g.filteredData[p.root], clause);
                        if (p.usePager) g.currentData = g._getCurrentPageData(g.filteredData); else g.currentData = g.filteredData;
                    }
        ...
        getColumn:function(columnParm) {
            ...
            if (typeof columnParm == "string") {
                if (g._isColumnId(columnParm)) return g._columns[columnParm]; else return g.columns[parseInt(columnParm)];
            } else if (typeof columnParm == "number") {
                return g.columns[columnParm];
            } else if (typeof columnParm == "object" && columnParm.nodeType == 1) {
        ...
        changeSort:function(columnName, sortOrder) {
            ...
            if (p.dataAction == "local") {
                ...
                if (!g.sortedData || !g.sortedData[p.root]) return;
                ...
                if (p.usePager) g.currentData = g._getCurrentPageData(g.sortedData); else g.currentData = g.sortedData;
                g._showData();
            }
        ...
        changePage:function(ctype) {
            ...
            switch (ctype) {
              ...
              case "input":
                ...
                if (isNaN(nv)) nv = 1;
                if (nv < 1) nv = 1; else if (nv > p.pageCount) nv = p.pageCount;
        ...
        getRowObj:function(rowParm, frozen) {
            ...
            if (typeof rowParm == "string") {
                if (g._isRowId(rowParm)) return document.getElementById(g.id + (frozen ? "|1|" :"|2|") + rowParm); else return document.getElementById(g.id + (frozen ? "|1|" :"|2|") + g.rows[parseInt(rowParm)]["__id"]);
            } else if (typeof rowParm == "number") {
                return document.getElementById(g.id + (frozen ? "|1|" :"|2|") + g.rows[rowParm]["__id"]);
            } else if (typeof rowParm == "object" && rowParm["__id"]) {
        ...
        getRow:function(rowParm) {
            ...
            if (rowParm == null) return null;
            if (typeof rowParm == "string") {
                if (g._isRowId(rowParm)) return g.records[rowParm]; else return g.rows[parseInt(rowParm)];
            } else if (typeof rowParm == "number") {
                return g.rows[parseInt(rowParm)];
            } else if (typeof rowParm == "object" && rowParm.nodeType == 1 && !rowParm["__id"]) {
        ...
        toggleCol:function(columnparm, visible, toggleByPopup) {
            ...
            if (visible && column._hide) {
                if (column.frozen) g.f.gridtablewidth += parseInt(colwidth) + 1; else g.gridtablewidth += parseInt(colwidth) + 1;
        ...
        changeCol:function(from, to, isAfter) {
            ...
            if (!sameParent) {
                ...
                if (isAfter) fromColumns.splice(fromColIndex, 1); else fromColumns.splice(fromColIndex + 1, 1);
            }
        ...
        getParent:function(rowParm) {
            ...
            if (rowdata["__pid"] in g.records) return g.records[rowdata["__pid"]]; else return null;
        },
        ...
        _addData:function(rowdata, parentdata, neardata, isBefore) {
            ...
            if (neardata) {
                if (p.tree) {
                    if (parentdata) listdata = parentdata[p.tree.childrenName]; else if (neardata["__pid"] in g.records) listdata = g.records[neardata["__pid"]][p.tree.childrenName];
                }
        ...
        _getColumnWidth:function(column) {
            ...
            if (typeof colwidth == "string" && colwidth.indexOf("%") > 0) {
                column._width = colwidth = parseInt(parseInt(colwidth) * .01 * (g.grid.width() - g.columns.length));
            }
        ...
        _createHeaderCell:function(column) {
            ...
            var headerText = "";
            if (column.display && column.display != "") headerText = column.display; else if (column.headerRender) headerText = column.headerRender(column); else headerText = "&nbsp;";
        ...
        _initBuildGridHeader:function() {
            ...
            for (var level = 1; level <= g._columnMaxLevel; level++) {
                ...
                $(columns).each(function(i, column) {
                    ...
                    if (column["__leaf"]) {
                        var colwidth = column["_width"];
                        if (!column.frozen) g.gridtablewidth += (parseInt(colwidth) ? parseInt(colwidth) :0) + 1; else g.f.gridtablewidth += (parseInt(colwidth) ? parseInt(colwidth) :0) + 1;
                    }
                });
            }
        ...
        _initBuildPopup:function() {
            ...
            if ($.fn.ligerCheckBox) {
                $("input:checkbox", g.popup).ligerCheckBox({
                    onBeforeClick:function(obj) {
                        ...
                        if ($("input:checked", g.popup).length <= p.minColToggle) return false;
                        return true;
                    }
                });
            }
            if (p.allowHideColumn) {
                ...
                if ($.fn.ligerCheckBox) $(":checkbox", g.popup).bind("change", onPopupCheckboxChange); else $(":checkbox", g.popup).bind("click", onPopupCheckboxChange);
            }
        },
        ...
        _fillGridBody:function(data, frozen) {
            ...
            if (g.enabledGroup()) {
                ...
                $(groupsdata).each(function(i, item) {
                    ...
                    if (i == groupsdata.length - 1) gridhtmlarr.push('<tr class="l-grid-grouprow l-grid-grouprow-last"'); else if (i == 0) gridhtmlarr.push('<tr class="l-grid-grouprow l-grid-grouprow-first"'); else gridhtmlarr.push('<tr class="l-grid-grouprow"');
                    ...
                    if (p.groupRender) gridhtmlarr.push(p.groupRender(groups[i], item, p.groupColumnDisplay)); else gridhtmlarr.push(p.groupColumnDisplay + ":" + groups[i]);
                    gridhtmlarr.push("</td>");
                    gridhtmlarr.push("</tr>");
        ...
        _showData:function() {
            ...
            if (p.usePager) {
                if (p.dataAction == "server" && g.data && g.data[p.record]) p.total = g.data[p.record]; else if (g.filteredData && g.filteredData[p.root]) p.total = g.filteredData[p.root].length; else if (g.data && g.data[p.root]) p.total = g.data[p.root].length; else if (data) p.total = data.length;
                p.page = p.newPage;
            ...
            for (var i = 0, l = data.length; i < l; i++) {
                ...
                $(g.columns).each(function(columnindex, column) {
                    ...
                    if (this.isrownumber) {
                        ...
                        if (p.fixedCellHeight) gridhtmlarr.push(' style = "height:' + p.rowHeight + 'px;" '); else gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;" ');
        ...
        _getCellHtml:function(rowdata, column) {
            var g = this, p = this.options;
            if (column.isrownumber) return '<div class="l-grid-row-cell-inner">' + (parseInt(rowdata["__index"]) + 1) + "</div>";
            var htmlarr = [];
        ...
        _getCellContent:function(rowdata, column) {
            ...
            if (column.isChildAttr && value) {
                value = value[column.childName];
            }
        ...
        _getTreeCellHtml:function(oldContent, rowdata) {
            ...
            if (isExtend && isParent) content += "<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-open'></div>"; else if (isParent) content += "<div class='l-grid-tree-space l-grid-tree-link l-grid-tree-link-close'></div>"; else content += "<div class='l-grid-tree-space'></div>";
        ...
        _applyEditor:function(obj) {
            ...
            if (column.editor.type && p.editors[column.editor.type]) {
                ...
                if ($.browser.mozilla) container.css({
                    left:left,
                    top:top
                }).show(); else container.css({
                    left:left,
                    top:top + 1
                }).show();
                if (column.textField) editParm.text = g._getValueByName(rowdata, column.textField);
        ...
        _compareData:function(data1, data2, columnName, columnType) {
            ...
            if (val1 == null && val2 != null) return 1; else if (val1 == null && val2 == null) return 0; else if (val1 != null && val2 == null) return -1;
            if (p.sorters[columnType]) return p.sorters[columnType].call(g, val1, val2); else return val1 < val2 ? -1 :val1 > val2 ? 1 :0;
        },
        _getTotalCellContent:function(column, data) {
            ...
            if (column.totalSummary) {
                ...
                if (column.totalSummary.render) {
                    ...
                    if (isExist("sum")) totalsummaryArr.push("<div>Sum=" + sum.toFixed(2) + "</div>");
                    if (isExist("count")) totalsummaryArr.push("<div>Count=" + count + "</div>");
                    if (isExist("max")) totalsummaryArr.push("<div>Max=" + max.toFixed(2) + "</div>");
                    if (isExist("min")) totalsummaryArr.push("<div>Min=" + min.toFixed(2) + "</div>");
                    if (isExist("avg")) totalsummaryArr.push("<div>Avg=" + avg.toFixed(2) + "</div>");
                }
            }
        ...
        _getTotalSummaryHtml:function(data, classCssName, frozen) {
            var g = this, p = this.options;
            var totalsummaryArr = [];
            if (classCssName) totalsummaryArr.push('<tr class="l-grid-totalsummary ' + classCssName + '">'); else totalsummaryArr.push('<tr class="l-grid-totalsummary">');
            $(g.columns).each(function(columnindex, column) {
        ...
        _setEvent:function() {
            ...
            $("select", g.toolbar).change(function() {
                if (g.isDataChanged && p.dataAction != "local" && !confirm(p.isContinueByDataChanged)) return false;
                p.newPage = 1;
        ...
        _onClick:function(e) {
            ...
            if (src.checkboxall) {
                ...
                g.selected = [];
                for (var rowid in g.records) {
                    if (uncheck) g.unselect(g.records[rowid]); else g.select(g.records[rowid]);
                }
        ...
        _onResize:function() {
            ...
            if (p.height && p.height != "auto") {
                ...
                if (typeof p.height == "string" && p.height.indexOf("%") > 0) {
                    ...
                    h = parentHeight * parseFloat(p.height) * .01;
        ...
        _stop:function(e) {
            ...
            if (g.xresize && g.xresize.diff != undefined) {
                ...
                if (g.dragtype == "leftresize") {
                    ...
                    if (g.center) g.center.width(g.center.width() + g.xresize.diff); else if (g.left) g.left.width(g.left.width() + g.xresize.diff);
                }
...
(function($) {
    ...
    $.ligerui.controls.ListBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _dataInit:function() {
            ...
            var valueArr = (value || "").toString().split(p.split);
            $("tr.l-selected", g.selectBox).removeClass("l-selected").find(":checkbox").each(function() {
                this.checked = false;
            });
            $(valueArr).each(function(i, item) {
                $("tr[value='" + item + "']", g.selectBox).addClass("l-selected").find(":checkbox").each(function() {
                    this.checked = true;
                });
            });
        },
        ...
        _addClickEven:function() {
            ...
            g.selectBox.click(function(e) {
                ...
                if (!p.isMultiSelect) {
                    $("tr.l-selected", g.selectBox).not(jrow).removeClass("l-selected").find(":checkbox").each(function() {
                        this.checked = false;
                    });
                }
                if (jrow.hasClass("l-selected")) {
...
(function($) {
    ...
    $.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
        ...
        updateShadow:function(menu) {
            ...
            if (menu.is(":visible")) menu.shadow.show(); else menu.shadow.hide();
        },
        hide:function(menu) {
            var g = this, p = this.options;
            if (menu == undefined) menu = g.menu;
        ...
        createMenu:function(parentMenuItemID) {
            ...
            if (parentMenuItemID) g.menus[parentMenuItemID] = menu; else g.menus[0] = menu;
            return menu;
        }
    });
    ...
    $.ligerui.controls.MessageBox.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render:function() {
            ...
            $(".l-messagebox-buttons-inner", g.messageBox).css({
                marginLeft:parseInt((boxWidth - sumBtnWidth) * .5)
            });
            g._applyWindowMask();
            g._applyDrag();
            g._setImage();
            ...
            if (p.left != null) left = p.left; else p.left = left = .5 * ($(window).width() - width);
            if (p.top != null) top = p.top; else p.top = top = .5 * ($(window).height() - g.messageBox.height()) + $(window).scrollTop() - 10;
            if (left < 0) p.left = left = 0;
            if (top < 0) p.top = top = 0;
            g.messageBox.css({
                left:left,
...
(function($) {
    ...
    $.ligerui.controls.Radio.ligerExtend($.ligerui.controls.Input, {
        ...
        _doclick:function() {
            ...
            var formEle;
            if (g.input[0].form) formEle = g.input[0].form; else formEle = document;
    ...
    $.ligerui.controls.Resizable.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render:function() {
            ...
            g.target.mousemove(function(e) {
                if (p.disabled) return;
                g.dir = g._getDir(e);
                if (g.dir) g.target.css("cursor", g.dir + "-resize"); else if (g.target.css("cursor").indexOf("-resize") > 0) g.target.css("cursor", "default");
        ...
        _applyResize:function(applyResultBody) {
            ...
            if (!applyResultBody) {
                ...
                applyToTarget = true;
                if (!isNaN(parseInt(g.target.css("top")))) cur.top = parseInt(g.target.css("top")); else cur.top = 0;
                if (!isNaN(parseInt(g.target.css("left")))) cur.left = parseInt(g.target.css("left")); else cur.left = 0;
            }
            ...
            if (applyToTarget && p.animate) applyResultBody.animate(cur); else applyResultBody.css(cur);
        }
    });
})(jQuery);
...
(function($) {
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        overrideTabItem:function(targettabid, options) {
            ...
            if (showClose == false) $(".l-tab-links-item-close", tabitem).remove(); else {
        ...
        onResize:function() {
            ...
            if (g.tab.parent()[0].tagName.toLowerCase() == "body") {
                ...
                g.height = p.heightDiff + g.tab.parent().height() * parseFloat(p.height) * .01;
            }
...
(function($) {
    ...
    $.ligerui.controls.TextBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _setValue:function(value) {
            if (value != null) this.inputText.val(value);
        },
...
(function($) {
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setTreeLine:function(value) {
            if (value) this.tree.removeClass("l-tree-noline"); else this.tree.addClass("l-tree-noline");
        },
        _setUrl:function(url) {
        ...
        selectNode:function(selectNodeParm) {
            ...
            if (typeof selectNodeParm == "function") {
                ...
                var treenodedata = g._getDataNodeByTreeDataIndex(g.data, treedataindex);
                var treeitembody = $(">div:first", treeitem);
                if (p.checkbox) $(".l-checkbox", treeitembody).removeClass("l-checkbox-unchecked").addClass("l-checkbox-checked"); else treeitembody.addClass("l-selected");
                g.trigger("select", [ {
                    data:treenodedata,
                    target:treeitem[0]
                } ]);
        ...
        _addToNodes:function(data) {
            var g = this, p = this.options;
            g.nodes = g.nodes || [];
            if ($.inArray(data, g.nodes) == -1) g.nodes.push(data);
            if (data.children) {
        ...
        _getTreeHTMLByData:function(data, outlineLevel, isLast, isExpand) {
            ...
            var treehtmlarr = [];
            if (!isExpand) treehtmlarr.push('<ul class="l-children" style="display:none">'); else treehtmlarr.push("<ul class='l-children'>");
            for (var i = 0; i < data.length; i++) {
                var isFirst = i == 0;
                ...
                if (g.hasChildren(o)) {
                    if (isExpandCurrent) treehtmlarr.push('<div class="l-box l-expandable-open"></div>'); else treehtmlarr.push('<div class="l-box l-expandable-close"></div>');
                    if (p.checkbox) {
                    ...
                    if (isLastCurrent) treehtmlarr.push('<div class="l-box l-note-last"></div>'); else treehtmlarr.push('<div class="l-box l-note"></div>');
                    if (p.checkbox) {
                        if (o.ischecked) treehtmlarr.push('<div class="l-box l-checkbox l-checkbox-checked"></div>'); else treehtmlarr.push('<div class="l-box l-checkbox l-checkbox-unchecked"></div>');
                    }
                    if (p.childIcon) {
        ...
        _setTreeEven:function() {
            ...
            g.tree.click(function(e) {
                ...
                if ($(obj).hasClass("l-checkbox")) {
                    ...
                    if (p.slide) $("> .l-children", treeitem).slideToggle("fast"); else $("> .l-children", treeitem).toggle();
...
(function($) {
    ...
    l.controls.Window.ligerExtend(l.core.Win, {
        ...
        _render:function() {
            ...
            var left = 0;
            var top = 0;
            if (p.left != null) left = p.left; else p.left = left = .5 * ($(window).width() - g.window.width());
            if (p.top != null) top = p.top; else p.top = top = .5 * ($(window).height() - g.window.height()) + $(window).scrollTop() - 10;
            if (left < 0) p.left = left = 0;
            if (top < 0) p.top = top = 0;
            g.set(p);
